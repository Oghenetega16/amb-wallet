...
export default function CurrenciesPage() {
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: "#060d1f" }}>
      ...
      <div className="flex flex-col flex-1 min-w-0 relative z-10 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto px-3 md:px-5 py-4">
          <div className="max-w-[1400px] mx-auto">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-5">
              <div className="relative flex-1 sm:max-w-xs">
                ...
                <input value={query} onChange={e => setQuery(e.target.value)}
                  placeholder="Search coins..." className="field-input pl-9 py-2 text-sm w-full" />
              </div>
              <div className="flex gap-1.5 flex-wrap">
                {(["value","price","change","marketCap"] as SortKey[]).map(s => {
                  const labels: Record<SortKey, string> = { value:"Holdings", price:"Price", change:"24h", marketCap:"Mkt Cap" };
                  return (
              ...
              <div className="flex items-center gap-2 sm:ml-auto">
                {lastUpdated && <p className="text-[10px] hidden sm:block" style={{ color: "#3d5070" }}>Updated {lastUpdated}</p>}
            ...
            {isError && <div className="mb-4"><ErrorState variant="fetch" compact onRetry={() => refetch()} /></div>}
            ...
            <div className="hidden md:block">
              ...
              <motion.div variants={staggerContainer} initial="hidden" animate="show" className="flex flex-col gap-2">
                {isLoading
                  ? Array.from({length:6}).map((_,i) => <div key={i} className="glass-card px-4 py-1"><CoinRowSkeleton /></div>)
                  : filtered.map(coin => {
                      const isPos = coin.priceChangePct24h >= 0;
                      const sparkData = coin.sparkline.map((v,i) => ({i,v}));
                      return (
                        <motion.div key={coin.id} variants={fadeUp} whileHover={{ scale: 1.003, x: 2 }}
                          ...
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold shrink-0"
                              style={{ background: coin.bgColor, color: coin.color }}>{coin.symbol.slice(0,2)}</div>
                            <div>
                              <p className="text-sm font-semibold text-white">{coin.symbol}</p>
                              <p className="text-xs" style={{ color: "#6b7fa8" }}>{coin.name}</p>
                            </div>
                          </div>
                          <motion.p key={coin.price.toFixed(2)}
                            initial={{ color: isPos?"#22d3a5":"#f75f7b" }} animate={{ color: "#e8edf8" }}
                          ...
                          <div>
                            ...
                            <p className="text-[10px]" style={{ color: "#6b7fa8" }}>{coin.quantity.toFixed(4)} {coin.symbol}</p>
                          </div>
                          <div className="h-10">
                            <ResponsiveContainer width="100%" height="100%">
                              <LineChart data={sparkData}>
                                <Line type="monotone" dataKey="v" stroke={isPos?"#22d3a5":"#f75f7b"} strokeWidth={1.5} dot={false} isAnimationActive={false}/>
                              </LineChart>
                            </ResponsiveContainer>
                          </div>
                          <p className="text-sm text-white">{coin.marketCap ? formatCompactNumber(coin.marketCap) : "—"}</p>
                          <button onClick={e => { e.stopPropagation(); openModal("send", coin); }}
            ...
            <div className="md:hidden flex flex-col gap-3">
              {isLoading
                ? Array.from({length:4}).map((_,i) => <div key={i} className="glass-card px-4 py-1"><CoinRowSkeleton /></div>)
                : filtered.map(coin => {
                    const isPos = coin.priceChangePct24h >= 0;
                    return (
                      <motion.div key={coin.id} variants={fadeUp}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs" style={{ color: "#6b7fa8" }}>{coin.quantity.toFixed(4)} {coin.symbol}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-bold text-white">{formatCurrency(coin.price)}</p>
                          <p className={`text-xs font-semibold ${isPos?"text-accent-green":"text-accent-red"}`}>
                            {formatPercent(coin.priceChangePct24h)}
                          </p>
                        </div>
                        <button onClick={e => { e.stopPropagation(); openModal("send", coin); }}
                          className="px-3 py-1.5 rounded-lg text-xs font-semibold ml-1 shrink-0"
                          style={{ background:"rgba(79,142,247,0.1)", color:"#4f8ef7", border:"1px solid rgba(79,142,247,0.2)" }}>
                          Trade
                        </button>
                      </motion.div>
      ...
      <SendModal /><ReceiveModal /><CoinDetailModal />
    </div>
  );
}
